...
void DialogueBox::setupUI() {
    setMinimumWidth(600);
    setMinimumHeight(150);  // Reduced minimum height
    setMaximumHeight(150);  // Added maximum height

    mainLayout = new QVBoxLayout(this);
    mainLayout->setSpacing(5);  // Reduced spacing
    mainLayout->setContentsMargins(0, 0, 0, 0);

    // Create dialogue container with dark, semi-transparent background
    ...
    containerLayout->setSpacing(2);  // Reduced spacing
    containerLayout->setContentsMargins(10, 5, 10, 5);  // Reduced margins
...
void DialogueBox::setNextDialogue(const QString &text, const QString &speaker) {
    nextText = text;
    nextSpeaker = speaker;
    // Show the next button even if there's no next text, as long as we're emitting dialogueFinished
    nextButton->setVisible(true);
}

void DialogueBox::onNextClicked() {
    emit dialogueFinished();  // Always emit to trigger next message
    if (!nextText.isEmpty()) {
