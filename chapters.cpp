...
void Chapters::setupUI()
{
    ...
    outerContainer1->setObjectName("chapter1Outer"); // Add this line
    ...
    // Create a container for the green box and description
    ...
    contentLayout1->addStretch(); // Add stretch to push content up
    ...
    checkmark1->hide();
    checkmark1->setCursor(Qt::PointingHandCursor);  // Add cursor change on hover
    checkmark1->installEventFilter(this);  // Install event filter to handle clicks

    checkmarkLayout->addWidget(checkmark1);
    outerLayout1->addWidget(checkmarkContainer);
    outerLayout1->addStretch();

    // Create other chapters (2-5) with dark containers
    auto createLockedChapter = [this](const QString &number, const QString &bgColor,
        ...
        // Add opacity effect for locked chapters
...
{
    // Implementation for future chapter descriptions
}
